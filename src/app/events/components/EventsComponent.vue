...
import SearchEventComponent from '@/app/events/components/SearchEventComponent.vue'
...
<template>
  <section class="my-events">
    <template v-if="loading">
      <!-- Aquí se muestra el esqueleto de carga mientras se están cargando los eventos -->
    ...
    <DataView v-else :value="filteredEvents" :layout="layout" paginator :rows="8">
      <template #header>
        <div class="flex justify-content-between gap-4">
          <SearchEventComponent @update-search="updateSearch" @update-continent="updateCategory" />
          <DataViewLayoutOptions v-model="layout" />
        </div>
      </template>
              ...
              <div class="flex flex-column md:flex-row justify-content-between md:align-events-center flex-1 gap-4">
                ...
                <div class="flex flex-column md:align-items-end gap-5">
                  ...
                  <div class="flex flex-row-reverse md:flex-row gap-2">
                    ...
                    <Button icon="pi pi-shopping-cart" label="Get Ticket" class="btn flex-auto md:flex-initial white-space-nowrap" @click="goToEventDetail(event)"/>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div class="pt-4">
                ...
                <div class="flex flex-column gap-4 mt-4">
                  <div class="flex gap-2">
                    <Button icon="pi pi-bookmark" outlined v-if="authStore.isLoggedIn"/>
                    <Button class="btn" icon="pi pi-share-alt"/>
                    <Button icon="pi pi-shopping-cart" label="Get Ticket" :disabled="event.inventoryStatus === 'OUTOFSTOCK'" class="btn flex-auto white-space-nowrap" @click="goToEventDetail(event)"></Button>
                  </div>
                </div>
              </div>
            </div>
